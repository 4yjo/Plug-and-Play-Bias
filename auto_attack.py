...
runs = api.runs(path=f"plugandplay/TargetsMouthopenEyeglasses")
...
for idx, run in enumerate(run_ids):
    command = f"python ./attack.py -c=configs/attacking/CelebA_Attr.yaml --wandb_target_run=\"TargetsMouthopenEyeglasses/{run}\" --run_name=\"Glasses_{idx/10}\""
